...
import { getMyArchivedListings } from '@/services/userApi';
...
onMounted(async () => {
  try {
    listings.value = await getMyArchivedListings()
  } catch (e: any) {
...
<template>
  ...
  <main>
    <div class="mylistings-container">
      <h4>{{ t('Your Archived Listings') }}</h4>
      ...
      <p v-if="listings.length === 0">{{ t('You have not archived any listings yet.') }}</p>
    </div>
  </main>
</template>
